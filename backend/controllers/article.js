...
router.get("/top_read", function (req, res, next) {
  ...
  dbUtils
    .query({ sql: indexSQL.GetTopRead, values: [Number(params.count)] })
    .then(({ results }) => {
      if (results) {
        res.send({
          code: "0",
...
router.get("/neighbors", function (req, res, next) {
  ...
  dbUtils
    .query({ sql: indexSQL.QueryPreAndNextArticleIds, values: [id, id] })
    .then(({ results }) => {
      if (results) {
        res.send({
          code: "0",
...
router.put("/update_read_num", function (req, res, next) {
  dbUtils
    .query({ sql: indexSQL.UpdateReadSum, values: [req.body.id] })
    .then(({ results }) => {
      if (results) {
        res.send({
          code: "0",
...
router.put("/update_private", function (req, res, next) {
  ...
  dbUtils
    .query({
      ...
      values: [params.private, params.id],
    })
    .then(({ results }) => {
      if (results) {
        res.send({
...
router.put("/update_deleted", function (req, res, next) {
  ...
  dbUtils
    .query({
      ...
      values: [params.deleted, params.id],
    })
    .then(({ results }) => {
      if (results) {
        res.send({
...
router.delete("/delete", function (req, res, next) {
  const params = req.query;
  dbUtils
    .query({ sql: indexSQL.DeleteArticleById, values: [params.id] })
    .then(({ results }) => {
      if (results) {
        res.send({
          code: "0",
...
router.get("/detail", function (req, res, next) {
  const params = req.query;
  dbUtils
    .query({ sql: indexSQL.GetArticleByID, values: [Number(params.id)] })
    .then(({ results }) => {
...
router.get("/page_by_tag", function (req, res, next) {
  ...
  dbUtils
    .query({
      ...
      values: [req.query.keyword, (pageNo - 1) * pageSize, pageSize],
    })
    .then(({ results }) => {
      if (results) {
        results[0].forEach(handleCategoryAndTag);
        ...
        children: [
            ...
            ? {
                ...
                task: function () {
                  const addCategoryTaskList = params.newCategories.map((item) =>
                    dbUtils.query(
                      { sql: indexSQL.AddCategories, values: [item] },
                      connection,
                      false
                    )
                  );
                ...
                children: [
                  {
                    // 任务1-2-1：插入文章分类关系表
                    task: function () {
                      const addArticleCategoryTaskList = newCategoryIds.map(
                        (item) =>
                          dbUtils.query(
                            {
                              sql: indexSQL.AddArticleCategory,
                              values: [articleId, item],
                            },
                            connection,
                            false
                          )
          ...
          params.oldCategoryIds
            ? {
                ...
                task: function () {
                  const addArticleCategoryTaskList = params.oldCategoryIds.map(
                    (item) =>
                      dbUtils.query(
                        {
                          sql: indexSQL.AddArticleCategory,
            ...
            : null,
          {
            ...
            task: function () {
              const addTagTaskList = params.tags.map((item) =>
                dbUtils
                  .query(
                    { sql: indexSQL.CheckTag, values: [item] },
                    connection,
                    false
                  )
                  .then(({ results }) => {
                    if (results.length == 0) {
                      ...
                      return dbUtils
                        .query(
                          { sql: indexSQL.AddTags, values: [item] },
                          connection,
                          false
                        )
                        .then(({ results }) => {
    ...
    const allTaskList = [
          ...
          return dbUtils.query(
            {
              ...
              values: [updateArticleParam, params.id],
            },
            connection,
            false
          );
        ...
        ? {
            ...
            task: function () {
              const deleteArticleTagTaskList = params.deleteTagIDs.map((item) =>
                dbUtils.query(
                  { sql: indexSQL.DeleteArticleTag, values: [params.id, item] },
                  connection,
                  false
                )
              );
      ...
      params.deleteCategoryIDs
        ? {
            // 任务4
            task: function () {
              const deleteArticleCategoryTaskList =
                params.deleteCategoryIDs.map((item) =>
                  dbUtils.query(
                    {
                      sql: indexSQL.DeleteArticleCategory,
                      values: [params.id, item],
                    },
                    connection,
                    false
        ...
        ? {
            ...
            task: function () {
              const addCategoryTaskList = params.tags.map((item) =>
                dbUtils
                  .query(
                    { sql: indexSQL.CheckCategory, values: [item] },
                    connection,
                    false
                  )
                  .then(({ results }) => {
